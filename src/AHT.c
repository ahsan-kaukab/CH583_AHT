...
HAL_StatusTypeDef AHT21_init(void) {
	...
	buff[0] = 0x71;
	i2c_write_to(AHT_21_ADDR, buff, 1,200,0);
	/*ret = HAL_I2C_Master_Transmit(&AHT21_I2C_PORT, AHT_21_ADDR, buff, 1,
			i2c_RETRY_TIME);
	if (ret != HAL_OK) {
		return HAL_ERROR;
	}*/
	i2c_read_from(AHT_21_ADDR, buff, 1,0, 2000);
...
uint32_t AHT21_Read_Humidity(void) {
	...
	buff[1] = 0X33;
	buff[2] = 0x00;
	i2c_write_to(AHT_21_ADDR, buff, 3,200,0);
	...
	mDelaymS(100);
	i2c_read_from(AHT_21_ADDR, buff, 6,0, 2000);
